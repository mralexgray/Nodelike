...
- init { return self = [super init] ? [NLContext attachToContext:self], self : nil; }

- initWithVirtualMachine:(JSVirtualMachine *)virtualMachine {

  return self = [super initWithVirtualMachine:virtualMachine] ?
  [NLContext attachToContext:self], self : nil;
}

+ (NLContext *)currentContext { return (NLContext *)[super currentContext]; }

#pragma mark - Scope Setup MEAT + POTATOES
...
#define NANOS_PER_SEC ((unsigned int)1000000000)
  ...
  /*! hrtime exposes libuv's uv_hrtime() high-resolution timer.
      ...
      so this function instead returns an Array with 2 entries representing seconds and nanoseconds, 
  ...
  process[@"hrtime"] = ^(JSValue *offset) {
    ...
    if (!offset.isUndefined && offset.isObject) {
      ...
      uint64_t seconds = [offset valueAtIndex:0].toInt32,
                 nanos = [offset valueAtIndex:1].toInt32;
